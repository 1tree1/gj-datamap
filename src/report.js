...
async function main_() {
  ...
  if (MS?.religion) add({ id: "religion", g: "people", t: `종교시설 ${fmt(MS.religion.n)}곳 — 종교별`, take: `${MS.religion.rows.slice(0, 3).map((r) => `${r[0]} ${r[1]}곳`).join(", ")}. 이슬람 시설은 시의 현황 자료에 항목이 없다.`, tier: "T2", src: MS.religion.src, lead: "경주시 종교시설현황(2025.02) 파일이다. 외국인주민 22,467명 중 이슬람권 국적(우즈베키스탄·카자흐스탄·인도네시아·방글라데시·파키스탄 등)이 3,000명 이상이지만, 이 자료에는 이슬람 시설 항목이 없다.",
...
function render(R, X, MS, Y) {
  ...
  document.getElementById("counter-body").innerHTML = `<table class="t"><tr><th>반대 의견 · 불확실한 점</th><th>대응 · 현재 상태</th></tr>
    ...
    <tr><td>시청은 낮에만 사람이 있어 밤에는 빈다</td><td>맞는 지적이다. 그래서 청년 주거와 체류형 숙박이 같은 부지에 있어야 한다. 시청만 두는 안은 없다. 20–34세 순유출과 황남동 청년 비율 7%가 그 수요의 크기를 보여준다.</td></tr>
    <tr><td>2015년 설문 1위는 복합위락시설(44.5%)이었다</td><td>맞는 지적이다. 쇼핑 수요는 마켓홀과 소매점으로, 위락 수요는 체류형 숙박과 야시장으로 받는다. 시청은 그 수요를 만들어 내는 배후 인구 역할이다.</td></tr>
    <tr><td>공공청사라면 높이 제한을 피해 상징 건물을 지을 수 있지 않나</td><td>불가능하다. 20m 상한은 문화유산 주변 높이 기준을 받은 값이라 공공청사도 예외가 없다. 상징성은 광장, 옛 역사(등록문화유산), 수장고 같은 요소로 만든다.</td></tr>
